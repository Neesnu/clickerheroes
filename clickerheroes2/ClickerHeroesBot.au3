...
Global $arrHeroes[30][5]= _
		[ _
		["cid.png",0,0,0,0],["tree.png",0,10,100,100],["ivan.png",0,0,0,0], _
		["brittany.png",0,0,0,0],["fish.png",0,10,100,100], ["betty.png",0,10,100,100], _
		["samurai.png",0,10,900,1750], ["leon.png",0,10,100,100], ["forest.png",0,0,0,0], _
		["alexa.png", 0,0,0,0], ["natalia.png",0,0,0,0], ["mercedes.png",0,10,100,100], _
		["bobby.png", 0,0,0,0], ["broyle.png",0,10,100,100], ["george.png",0,0,0,0], _
		["king.png",0,25,100,100], ["jerator.png",0,0,0,0], ["abaddon.png",0,0,0,0], _
		["zhu.png",0,0,0,0], ["amen.png",0,25,200,200], ["beast.png",0,25,100,100], _
		["athena.png",0,0,0,0], ["aphrodite.png",0,0,0,0], ["shinatobe.png",0,25,100,100], _
		["grant.png",0,25,100,100], ["frostleaf.png",0,100,100,200], ["atlas.png",0,0,0,1300], _
		["temp.png",0,0,0,0], ["temp.png",0,0,0,0], ["temp.png",0,0,0,0] _
		]
Global $curHero = 0
Global $result = 0
...
Func levelUp()
   ...
   Case 26
	  ...
	  EndIf
   Case 27
	  If $arrHeroes[26][1] >= $arrHeroes[26][4] Then
		 Sleep(60000)
		 Ascend()
	  EndIf
	  levelUpEndGame()
...
Func findHero($pos)
   ...
   If ($pos = 0) Then
	  ...
	  Wend
   ;Other Heroes
   ElseIf (($arrHeroes[$pos][1] > 0 or $arrHeroes[$pos][2] > 0 or $arrHeroes[$pos][4] > 0)) Then
	  While ($i = 0)
		 $result = _ImageSearchArea("images/heroes/normal/" & $arrHeroes[$pos][0], 1, $left, $top, $right, $bottom, $x1, $y1, 60)
		 If $result = 1 Then
			If $y1 < 200 Then
...
Func hireLevel($pos)
   ...
   ;Hire
	  If ($arrHeroes[$pos][1]= 100000) Then
		 Sleep(100)
	  ...
	  If ($arrHeroes[$pos][2] > 0) Then
		 ...
		 Do
			...
			If $result = 1 Then
			   ...
			   If $result = 1 Then
				  $result = _ImageSearchArea("images/yellow.png", 1, $x2, $y2, $x2 + 40, $y2 + 40, $x1, $y1, 30)
				  If $result = 1 Then
					 ;MouseClick("left",$x2,$y2)
					 WinActivate("Clicker Heroes")
					 ControlClick("Clicker Heroes","", "", "Left",1,$x2,$y2)
					 $arrHeroes[$pos][1]= $arrHeroes[$pos][1] + 25
					 ...
					 EndIf
				  Else
					 $result = _ImageSearchArea("images/orange.png", 1, $x2, $y2, $x2 + 40, $y2 + 40, $x1, $y1, 30)
					 If $result = 0 Then
						WinActivate("Clicker Heroes")
						MouseWheel("up")
						Sleep(200)
			...
			Else
			   _GUICtrlEdit_AppendText($editctrl,"Hirelevel Cant Find Hero " & $arrHeroes[$pos][0] & @CRLF)
			   findHero($pos)
			EndIf
		 Until ($arrHeroes[$pos][1] >= $arrHeroes[$pos][2] or $intCounter > 120)
	  ElseIf ($arrHeroes[$pos][2] = 0) Then
		 $curHero = $pos + 1
	  EndIf
EndFunc
...
Func levelUpEndGame()
   ...
   ; Fill the array with data.
    For $i = 0 To $intHeroCount - 1
      If (($arrHeroes[$i][1] > 0  and ($arrHeroes[$i][1] < $arrHeroes[$i][4])) or ($i = 26 and ($arrHeroes[$i][1] < $arrHeroes[$i][4]))) Then
		 If $doOnce = 0 Then
			MouseWheel("up",20)
			$doOnce = 1
			Sleep(300)
		 ...
		 EndIf
		 findHero($i)
		 _GUICtrlEdit_AppendText($editctrl,"Leveling up to EndGame " & $arrHeroes[$i][4] & "Hero: " & $arrHeroes[$i][0] & @CRLF)
		 Local $loopOnce = 0
		 Do
			$result = _ImageSearchArea("images/heroes/normal/" & $arrHeroes[$i][0], 1, $left, $top, $right, $bottom, $x1, $y1, 60)
			If $result = 1 Then
			   ...
			   If $result = 1 Then
				  ...
				  If $result = 1 Then
					 ...
					 checkFarmState()
					 findFish()
					 If $i = 26 and ($arrHeroes[$i][1] = 50 or $arrHeroes[$i][1] = 100) Then
						WinActivate("Clicker Heroes")
						MouseWheel("up")
						Sleep(200)
						MouseWheel("down",3)
			   ...
			   Else
				  If $loopOnce = 1 Then
					 ...
					 $loopOnce = 1
				  Else
					 MouseClick("left",$left+50,$top+50)
				  EndIf
			   EndIf

			   ControlSend ("Clicker Heroes","","","{Z UP}")
			EndIf
		 Until ($arrHeroes[$i][1] >= $arrHeroes[$i][4])
	  EndIf
    Next
...
do
   ...
   If ($arrHeroes[0][1]= 0) and ($arrHeroes[0][2] <> 0) Then
